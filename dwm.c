...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - 2 * sp, 0, tw, bh, 0, stext, 0, True);
	}
...
if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w - 2 * sp, bh, tlpad, m->sel->name, 0, False);
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs + tlpad, boxs, boxw, boxw, m->sel->isfixed, 0);
		} else {
